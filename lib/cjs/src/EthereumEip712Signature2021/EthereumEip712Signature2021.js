...
exports.getTypesForEIP712Domain = void 0;
const web3_1 = __importDefault(require("web3"));
//@ts-ignore
const suites_1 = require("jsonld-signatures/lib/suites");
//@ts-ignore
...
function getTypesForEIP712Domain(params) {
    return [
...
// @ts-ignore
JSON.canonify = jcs_1.JCS.cannonicalize;
class EthereumEip712Signature2021 extends suites_1.suites.LinkedDataSignature {
    index;
    web3;
    wallet;
    account;
    ...
    async generateKeyPair(seed) {
        if (seed) {
            ...
            // @ts-ignore
            this.mnemonic = await bip39.generateMnemonic(256, crypto_1.default.randomBytes);
        }
        const mnemonic = await ethers_1.Mnemonic.fromPhrase(this.mnemonic);
        const wallet = await ethers_1.HDNodeWallet.fromMnemonic(mnemonic, `m/44'/60'/0'/0/${this.index}`);
        this.wallet = wallet;
    ...
    async switchAccount(index) {
        const mnemonic = await ethers_1.Mnemonic.fromPhrase(this.getMnemonic());
        const wallet = await ethers_1.HDNodeWallet.fromMnemonic(mnemonic, `m/44'/60'/0'/0/${index}`);
        this.wallet = wallet;
        const account = await this.web3.eth.accounts.privateKeyToAccount(wallet.privateKey);
        this.account = account;
        return {
    ...
    async canonicalizationHash(message, options) {
        const c14nDocument = await jsonld_1.default.canonize(message, {
            algorithm: "URDNA2015",
    ...
    async createProof(options) {
        ...
        proof.verificationMethod = options.document.verificationMethod.find((obj) => obj.id === vmId);
    ...
    async sign(options) {
        ...
        if (this.web3._provider !== undefined && this.web3._provider !== null) {
            ...
            {
                //@ts-ignore
                proofValue = await (0, eth_sig_util_1.signTypedData_v4)(Buffer.from(this.wallet.privateKey.replace('0x', ''), 'hex'), { data: verifyData });
            }
        }
    ...
    verifySignature(options) {
        ...
        if (recoveredAddress.toLowerCase() ===
            this.extractAddressFromDID(options.verificationMethod, options.message.verificationMethod).toLowerCase()) {
            return true;
        }
        return false;
    }
    extractAddressFromDID(vmId, verificationMethod) {
        const vm = verificationMethod.find(vm => vm.id === vmId);
